...
class neuro(nn.Module):
    def __init__(self, n_c, n_b, scale):
        ...
        block = []
        self.conv_1 = nn.Conv2d(scale**2*3 + n_c + 3, n_c//2, (3,3), stride=(1,1), padding=pad)
        # self.conv_1 = nn.Conv2d(3, n_c//2, (3,3), stride=(1,1), padding=pad)
        ...
        # self.offset = ConvOffset2D(3)
        # self.dcn_0 = nn.Conv2d(3, n_c//2, (3,3), stride=(1,1), padding=pad)
        self.dcn_0 = BasicDeformConv2d(in_channels=3,
            out_channels=n_c // 2,
            kernel_size=3
        )
    ...
    def forward(self, x_i, x_i_prev, h, o):
        x = torch.cat((x_i.contiguous(), h, o), dim=1)
        x = F.relu(self.conv_1(x))

        # dbg("x_i_prev", x_i_prev.shape)
        # dbg("x_i_prev continguous", x_i_prev.contiguous().shape)
        x_prev = self.dcn_0(x_i_prev.contiguous())
        x_prev = F.relu(x_prev)
        # dbg("x_prev", x_prev.shape)
...
# https://pyup.io/changelogs/torchvision/
...
class RRN(nn.Module):
    ...
    def forward(self, x, x_h, x_o, init):
        ...
        f2 = x[:,:,1,:,:]
        if init:
            x_h, x_o = self.neuro(f2, f1, x_h, x_o)
        else:
            x_o = self.down(x_o)
            x_h, x_o = self.neuro(f2, f1, x_h, x_o)
